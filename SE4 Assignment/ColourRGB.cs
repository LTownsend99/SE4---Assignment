namespace SE4_Assignment
{
    public class ColourRGB : Shape
    {
        ...
        public override void runCommand(Draw draw)
        {
            ...
            try
            {
                red = int.Parse(parameters[0]);   // tries to parse the red as an int if not then an exception is thrown
                green = int.Parse(parameters[1]);  // tries to parse the green as an int if not then an exception is thrown
                blue = int.Parse(parameters[2]); // tries to parse the blue as an int if not then an exception is thrown
            }
